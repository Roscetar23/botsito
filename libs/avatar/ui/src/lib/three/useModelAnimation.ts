...
/**
 * Reproduce en bucle la animación del rig del GLB tal cual viene
 * (`useAnimations` de drei se encarga de avanzar el `AnimationMixer`
 * cada frame). El clip corregido en Blender ya solo mueve manos y cara —
 * el cuerpo queda quieto, así que no hace falta procesar/filtrar tracks.
 *
 * Usa el primer clip **por índice** (`animations[0]`, así no hace falta
 * escribir su nombre con acentos en el código, p. ej. `Esqueleto_acción`)
 * salvo que se pida uno por `clip` y exista. Con `playing=false` la deja
